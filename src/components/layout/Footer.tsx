        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          <div>
            ...
            <div className="space-y-2 text-sm text-white/70">
              <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-brand-orange shrink-0" /><span>{site.hours.weekdays}</span></div>
              <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-brand-orange shrink-0" /><span>{site.hours.weekend}</span></div>
            </div>
          </div>
          <div>
            ...
            <ul className="space-y-2">
              {services.map((s) => (
                <li key={s.slug}><Link href={`/services/${s.slug}`} className="text-sm text-white/70 hover:text-brand-orange transition-colors">{s.name}</Link></li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-2">
              {cities.map((c) => (
                <li key={c.slug}><Link href={`/service-areas/${c.slug}`} className="text-sm text-white/70 hover:text-brand-orange transition-colors">{c.name}, TX</Link></li>
              ))}
            </ul>
          </div>
          <div>
